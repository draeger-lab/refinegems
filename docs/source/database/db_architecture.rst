...
The diagram above shows the Entity-Relationship-diagram \(ER-diagram\) for the database.
The ER-diagram displays the four entities medium, substance, subset and database \(dark green\), their relations \(light green\) and attributes \(golden\). 
...
Similarly, each subset has a name and a description. It includes different substances with specific percentages.
...
Furthermore, the substances can be found in different databases (db) and connected to the respective identifiers (``1:n``). 
The database consists, thus, of the six tables: :ref:`medium`, :ref:`medium2substance`, :ref:`subset`, :ref:`subset2substance`, :ref:`substance` and :ref:`substance2db`.
...
Furthermore, since the chemical substance in the medium might differ from the compund used to create the medium, the column source provides the information about how and in which form the substance was added to the original medium.
...
Substances are added using the following naming conventions:
...
5. | Halogenic ions are saved in a similar manner,
   | e.g. Chloride \[Cl\(-\)\].

.. note::
    When converting the substances from the previous database format to this, well-annotated names were taken from the
...
The substance2db table contains the mapping of the substance identifiers to their respective various database identifiers. 
