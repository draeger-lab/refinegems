...
def save_user_input(configpath: str) -> dict[str: str]:
    ...
    if os.path.isfile(configpath):
        ...
        list_of_media = []
        while True:
            medium = click.prompt('Enter medium to simulate growth on (SNM3|LB|M9|SMM|CGXII|RPMI|Blood|Urine) (or "stop" to stop)')
            if medium.lower() == 'stop':
                break
            elif medium in ['SNM3', 'RPMI', 'CGXlab', 'LB', 'M9', 'CGXII', 'CasA', 'Blood', 'Urine']:
