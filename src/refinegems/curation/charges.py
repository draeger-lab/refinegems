...
def correct_charges_from_db(
    model: libModel, compounds: pd.DataFrame
) -> tuple[libModel, dict]:
    ...
    for i in spe:
        if not i.getPlugin(
            "fbc"
        ).isSetCharge():  # we are only interested in metab without charge
            bigg = i.getId()[2:-2]
            if (
                len(compounds[compounds["BiGG"] == bigg]["charge"].array) == 1
            ):  # eindeutig
                charge = compounds[compounds["BiGG"] == bigg]["charge"].array[0]
                i.getPlugin("fbc").setCharge(int(charge))
            elif len(compounds[compounds["BiGG"] == bigg]["charge"].array) > 1:
                charges = compounds[compounds["BiGG"] == bigg]["charge"].array
...
def correct_charges_modelseed(model: libModel) -> tuple[libModel, dict]:
    ...
    model_corr, multiple_charges = correct_charges_from_db(model, modelseed_compounds)
